  ...
  <v-card-text>
    ...
    <v-row>
      ...
      <v-col>
        ...
        <v-checkbox v-model="intersectPractices" dense label="Cruzar  practicas" />
      </v-col>
    </v-row>
    ...
    <v-dialog v-model="openMySchedules" fullscreen>
      ...
      <v-dialog
        ...
        max-width="500"
      >
        <SchedulesWelcome :show-welcome.sync="showWelcome" @close="closeWelcome()" />
      </v-dialog>
      <v-dialog
        v-model="openAlert"
        persistent
      ...
      >
        ...
        <template #top-items-right>
          <v-btn
            icon
            dark
            @click="openMyFavoritesSchedules = false"
          >
            <v-icon>mdi-close</v-icon>
          </v-btn>
...
export default class SchedulesGenerator extends Vue {
  ...
  generarTodosMisHorarios () {
    const { schedules, ocurrences } = generate(this.myCourses, this.myEvents, this.maxIntersections,
      this.intersectEvents, this.intersectPractices)
