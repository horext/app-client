...
import { subjectAggregatePatchSchema } from '../../../domain/schemas/subject.schema'
...
export default defineEventHandler(async (event) => {
  ...
  const result = await itemWriteUseCases(event).subjects.patch.execute({
    userId: auth.user.id,
    id: (await getValidatedRouterParams(event, idParamsSchema.parse)).id,
    value: await readValidatedBody(event, subjectAggregatePatchSchema.parse),
