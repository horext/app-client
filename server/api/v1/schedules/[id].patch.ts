...
import { scheduleAggregatePatchSchema } from '../../../domain/schemas/schedule.schema'
...
export default defineEventHandler(async (event) => {
  ...
  const result = await itemWriteUseCases(event).schedules.patch.execute({
    userId: auth.user.id,
    id: (await getValidatedRouterParams(event, idParamsSchema.parse)).id,
    value: await readValidatedBody(event, scheduleAggregatePatchSchema.parse),
